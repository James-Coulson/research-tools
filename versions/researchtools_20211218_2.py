...
def change_notebook_mode(status=True):
    """
    Used to change the notebook mode of plotly

    :param status: The new status (defaults to True)
    """
    init_notebook_mode(connected=status)


# ------------------------------------ Plotting Trade Data ------------------------------------


def plot_trade_data(symbols: list, start_date: str, end_date: str):
    """
    ...
    if len(symbols) > 1:
        ...
        for symbol in symbols:
            ...
            fig.append_trace(go.Scatter(x=trade_data['time'], y=trade_data['price'], name="{}".format(symbol)),
                             row=row_count, col=1)
            row_count += 1

        # Update title and print output
        # fig.update_layout(title_text="Trade data")
        fig.write_html('./outputs/trade_data.html', auto_open=True)
    else:
        # Create figure
        fig = px.line(get_binance_trade_data(start_date=start_date, end_date=end_date, symbols=[symbols[0]]), x='time',
                      y='price')
        fig.write_html('./trade_data.html', auto_open=True)


# ------------------------------------ Plotting Kline Data ------------------------------------


def plot_kline_data(symbols_intervals: list, start_date: str, end_date: str):
    """
    ...
    for (symbol, interval) in symbols_intervals:
        kline_data = get_binance_kline_data(start_date=start_date, end_date=end_date,
                                            symbol_data_required={symbol: [interval]})
        ...
        row_count += 1
    fig.write_html('./kline_data.html', auto_open=True)
